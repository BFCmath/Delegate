...
class MindSearchAgent:
    ...
    def forward(self, question: str) -> dict:
        ...
        # Add preamble explaining the research structure
        research_data = f"""# Deep Research Materials

## Original Research Topic
{question}

## Research Structure
Below you will find the complete research materials organized by research question. Each section contains:
1. The specific research question investigated
2. A synthesized answer based on web research
3. All source materials (URLs) that were crawled to gather this information

Your task is to synthesize ALL of this information into a comprehensive, professional research report.

**Important**: Do NOT include inline citations in your report. Write naturally and comprehensively based on all the research materials provided.
...
"""
    ...
    def _generate_references_from_graph(self, nodes: Dict[str, dict]) -> str:
        """
        Generate formatted references from graph nodes with ALL crawled data.
        
        For deep research, we include:
        1. The research question
        2. The synthesized answer (without inline citations)
        3. ALL raw source materials that were crawled
        """
        import re
        references = []
        ...
        for name, data in nodes.items():
            ...
            # Build the reference section for this node
            section_parts = []
            
            # 1. Research Question
            section_parts.append(f"### Research Question: {data['content']}\n")
            
            # 2. Synthesized Answer (remove inline citations for evaluation)
            content = data["response"]["content"]
            citations = data["response"].get("citations", {})
            
            # Remove all inline citations [[number]]
            cleaned_content = re.sub(r'\[\[\d+\]\]', '', content)
            # Clean up any multiple spaces that might result
            cleaned_content = re.sub(r'\s+', ' ', cleaned_content)
            cleaned_content = cleaned_content.strip()
            
            section_parts.append(f"**Answer:**\n{cleaned_content}\n")
            
            # 3. Raw Source Materials
            if citations:
                section_parts.append(f"\n**Source Materials ({len(citations)} sources):**")
                for idx in sorted(citations.keys(), key=int):
                    url = citations[idx]
                    section_parts.append(f"- {url}")
